...
SELECT MASV, HOSV +' '+TENSV AS HOTENSV, CONVERT(NVARCHAR, NGAYSINH, 103) AS NGSINH, PHAI
FROM DMSV
WHERE YEAR(NGAYSINH) = 1994 AND MONTH(NGAYSINH) BETWEEN 1 AND 3
...
----6.4
SELECT MASV, SUM(CASE WHEN LANTHI=1 AND DIEM <5 THEN 1 ELSE 0 END) AS LANTHILAI
FROM KETQUA
GROUP BY MASV
HAVING SUM(CASE WHEN LANTHI=1 AND DIEM <5 THEN 1 ELSE 0 END)>2
...
----6.7
SELECT M.MAMH, M.TENMH, COUNT(DISTINCT K.MASV) AS SOSV
...
WHERE DIEM = ( SELECT MAX( DIEM)
				FROM DMMH AS M, KETQUA AS K
				WHERE M.MAMH = K.MAMH AND K.LANTHI =1 AND M.TENMH = N'Cơ sở dữ liệu' )

----B4_N2 4.2.3
SELECT *
FROM DMSV
WHERE YEAR(GETDATE()) - YEAR(NGAYSINH) = ( SELECT MAX(YEAR(GETDATE())-YEAR(S.NGAYSINH))
											FROM DMSV AS S, DMKHOA AS K
											WHERE K.MAKHOA = S.MAKHOA AND K.TENKHOA = N'Anh văn')
