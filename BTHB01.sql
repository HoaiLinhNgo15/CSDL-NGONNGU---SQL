...
----VÍ DỤ 6
SELECT MaNV, HoNV+' '+ TenLotNV+' '+ TenNV AS HoTenNV, Phai, Hesoluong
FROM NHANVIEN
WHERE MaNV LIKE 'A1%'
...
FROM NHANVIEN
WHERE Phai = N'Nữ'
ORDER BY TenNV, HoNV, Hesoluong DESC
...
---6. HÀM TẬP HỢP
----VÍ DỤ 15 ---TỔNG THỜI GIAN THỰC HIỆN ĐỀ ÁN ---HÀM SUM
SELECT SUM(ThoiGian) AS "TỔNG THỜI GIAN"
FROM PHANCONG

----VÍ DỤ 16 ---TRUNG BÌNH THỜI GIAN THỰC HIỆN ĐỀ ÁN --- HÀM AVG
SELECT AVG(ThoiGian) AS "Tb_THỜI GIAN"
FROM PHANCONG

----VÍ DỤ 17 --- ĐẾM LƯỢT NGƯỜI THAM GIA DỰ ÁN --- HÀM COUNT
SELECT COUNT(MaNV) AS "SỐ NV THAMGIA"
FROM PHANCONG

----VÍ DỤ 18 --- THỜI GIAN THAM GIA ĐỀ ÁN LỚN NHẤT --- HÀM MAX
SELECT MAX(ThoiGian) AS "TG_LỚN NHẤT"
FROM PHANCONG

----VÍ DỤ 19 --- THỜI GIAN THAM GIA ĐỀ ÁN NHỎ NHẤT --- HÀM MIN
SELECT MIN(ThoiGian) AS "TG_NHỎ NHẤT"
FROM PHANCONG

---7. HÀM TẬP HỢP VỚI GROUP BY
----VÍ DỤ 20 --- THỜI GIAN THỰC HIỆN NHỎ NHẤT CỦA TỪNG ĐỀ ÁN
SELECT MaDA, MIN(ThoiGian) AS TG_nho_nhat
FROM PHANCONG
GROUP BY MaDA

----VÍ DỤ 21 --- ĐẾM MỖI NGƯỜI THỰC HIỆN BAO NHIÊU ĐỀ ÁN
SELECT MaNV, COUNT(MaDA) AS SoDA_TH
FROM PHANCONG
GROUP BY MaNV
ORDER BY SoDA_TH

----THỜI GIAN LỚN NHẤT TỪNG ĐỀ ÁN
SELECT MaDA, MAX(ThoiGian) AS max_time
FROM PHANCONG
GROUP BY MaDA
ORDER BY max_time

----THỜI GIAN TRUNG BÌNH TỪNG ĐỀ ÁN
SELECT MaDA, AVG(ThoiGian) AS TB_time
FROM PHANCONG
GROUP BY MaDA

----TỔNG THỜI GIAN THỰC HIỆN TỪNG ĐỀ ÁN
SELECT MaDA, SUM(ThoiGian) AS TONG_time
FROM PHANCONG
GROUP BY MaDA

----SỐ NGƯỜI THỰC HIỆN TỪNG ĐỀ ÁN
SELECT MaDA, COUNT(MaNV) AS SoNV_THAMGIA
FROM PHANCONG
GROUP BY MaDA

---8.CÚ PHÁP VỚI HAVING
----VÍ DỤ 22
SELECT MaNV, COUNT(MaDA) AS SoDuAn_NV
FROM PHANCONG
GROUP BY MaNV
HAVING MaNV LIKE 'A1%'
ORDER BY SoDuAn_NV
...
---9.ĐIỀU KIỆN KẾT NỐI
----VÍ DỤ  24
SELECT HoNV, TenLotNV AS TenLot, TenNV AS Ten, C.TenTDCM
FROM NHANVIEN AS N, CHUYENMON AS C
WHERE C.MaTDCM = N.MaTDCM

----VÍ DỤ 25
SELECT HoNV, TenLotNV AS TenLot, TenNV AS Ten, C.TenTDCM
FROM NHANVIEN AS N, CHUYENMON AS C
WHERE C.MaTDCM = N.MaTDCM AND C.TenTDCM like N'Kỹ sư Công nghệ thông tin'

----VÍ DỤ 26 ---UNION HOẶC UNION ALL ---PHÉP NỐI
SELECT N.MaNV, N.TenLotNV, N.TenNV
FROM NHANVIEN AS N, PHANCONG AS P, DEAN AS D
WHERE N.MaNV = P.MaNV AND P.MaDA = D.MaDA 
UNION
SELECT N.MaNV, N.TenLotNV, N.TenNV
FROM NHANVIEN AS N

----VÍ DỤ 27 ----INTERSECT ---PHÉP GIAO
SELECT N.MaNV, N.TenLotNV, N.TenNV
FROM NHANVIEN AS N, PHANCONG AS P, DEAN AS D
WHERE N.MaNV = P.MaNV AND P.MaDA = D.MaDA
INTERSECT 
...
----VÍ DỤ 29 --- CÚ PHÁP IN ---TƯƠNG TỰ NHIỀU LỆNH OR
SELECT MaNV, HoNV, TenLotNV, TenNV, Hesoluong, MaTDCM
FROM NHANVIEN
WHERE MaTDCM IN ('A01', 'A02', 'A03')
...
WHERE MaTDCM = SOME (
SELECT MaTDCM 
FROM NHANVIEN AS A, NHAN_NHIEMVU AS B, PHONGBAN AS C
WHERE A.MaNV = B.MaNV AND B.MaPB = C.MaPB AND C.TenPB = N'Kỹ thuật'
)
